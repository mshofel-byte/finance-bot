...
function formatAmount(n) {
  return Number(n).toLocaleString("he-IL") + " CZK";
}
...
const KOLIN_INVESTORS =
  "משקיעי Czech Israel קולין (19 משקיעים)\n" +
  "סה\"כ: 60,227,237 CZK\n\n" +
  "יהודה חפץ - 3,070,533 CZK\n" +
  "יואל דרקסלר - 3,070,533 CZK\n" +
  "נתי מצגר - 3,070,533 CZK\n" +
  "מוטי ויטמן - 3,070,533 CZK\n" +
  "עידן מאירוביץ - 3,070,533 CZK\n" +
  "אופיר נחום - 6,141,066 CZK\n" +
  "אברהם אזוט - 1,562,500 CZK\n" +
  "שר שלום בוטה - 1,562,500 CZK\n" +
  "חיים כרמל - 3,125,000 CZK\n" +
  "יעקב ברויער - 1,562,500 CZK\n" +
  "דני ברויער - 1,562,500 CZK\n" +
  "יששכר בקר - 3,125,000 CZK\n" +
  "דוד אזוט - 1,250,000 CZK\n" +
  "הבוטקה - 1,875,000 CZK\n" +
  "גבעת המצור - 13,852,656 CZK\n" +
  "הרב משה שופל - 4,581,250 CZK\n" +
  "חגי ליפמן - 1,875,000 CZK\n" +
  "אורן פנצר - 2,500,000 CZK\n" +
  "הרב שפירא - 2,500,000 CZK";
...
const SYSTEM_PROMPT =
  ...
  "משקיעי קולין (19 משקיעים, סה\"כ 60,227,237 CZK):\n" +
  "יהודה חפץ, יואל דרקסלר, נתי מצגר, מוטי ויטמן, עידן מאירוביץ - כל אחד 3,070,533 CZK\n" +
  "אופיר נחום - 6,141,066, אברהם אזוט - 1,562,500, שר שלום בוטה - 1,562,500\n" +
  "חיים כרמל - 3,125,000, יעקב ברויער - 1,562,500, דני ברויער - 1,562,500\n" +
  "יששכר בקר - 3,125,000, דוד אזוט - 1,250,000, הבוטקה - 1,875,000\n" +
  "גבעת המצור - 13,852,656, הרב משה שופל - 4,581,250\n" +
  "חגי ליפמן - 1,875,000, אורן פנצר - 2,500,000, הרב שפירא - 2,500,000\n\n" +
...
function getCompanyMenu(companyKey) {
  if (companyKey === "company_kolin") {
    return {
      reply_markup: {
        inline_keyboard: [
          [
            { text: "סטטוס ולוח זמנים", callback_data: "company_kolin_status" },
            { text: "תוכנית עסקית", callback_data: "company_kolin_finance" },
          ],
          [
            { text: "משקיעים", callback_data: "company_kolin_investors" },
            { text: "תזרים", callback_data: "company_kolin_cashflow" },
          ],
...
bot.on("callback_query", async function(query) {
  ...
  if (data === "company_kolin_investors") {
    bot.editMessageText(KOLIN_INVESTORS, { chat_id: chatId, message_id: query.message.message_id, reply_markup: getCompanyMenu("company_kolin").reply_markup });
    bot.answerCallbackQuery(query.id);
    return;
  }
  ...
  if (data.indexOf("cf_") === 0) {
    ...
    const cfType = parts[parts.length - 2] + "_" + parts[parts.length - 1];
  ...
  if (data.indexOf("rpt_") === 0) {
    ...
    const companyKey = parts.slice(1, -1).join("_");
  ...
  const actionMap = { status: "סטטוס עדכני ולוח זמנים", finance: "תוכנית עסקית", timeline: "לוח זמנים", contacts: "אנשי קשר" };
