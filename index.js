...
const result = await pool.query(
“SELECT type, SUM(amount) as total FROM cashflow WHERE 1=1 “ + companyFilter + “ “ + dateFilter + “ GROUP BY type”
);
...
return (
“דוח תזרים “ + periodLabels[period] + “ - “ + companyName + “\n\n” +
“צפוי:\n” +
...
if (ext === “.xlsx” || ext === “.xls” || (mimeType && (mimeType.includes(“spreadsheet”) || mimeType.includes(“excel”)))) {
...
const prompt = “האם הטקסט הבא מכיל הזנת תזרים? טקסט: "” + text + “"\n” +
“חברות: company_kolin=Czech Israel קולין, company_cbrmy=CBRMY, company_besiata=BESIATA, company_beezrato=BEEZRATO, company_holding=Czech Israel הולדינג\n” +
...
bot.editMessageText(“תזרים מרוכז - כל החברות\nבחר תקופה:”, { chat_id: chatId, message_id: query.message.message_id, …getMergedCashflowMenu() });
bot.answerCallbackQuery(query.id);
return;
}
...
bot.editMessageText(“תזרים - “ + companyName + “\nבחר פעולה:”, { chat_id: chatId, message_id: query.message.message_id, …getCashflowMenu(companyKey) });
bot.answerCallbackQuery(query.id);
return;
}
...
userState[chatId] = { action: “cashflow_entry”, companyKey, type: cfType, companyName, typeLabel };
bot.answerCallbackQuery(query.id);
...
bot.sendChatAction(chatId, “typing”);
try {
const msg = await buildReport(companyKey, period);
